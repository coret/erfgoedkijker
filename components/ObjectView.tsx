...
import { PROFILE, classLabel, propertyDescription, propertyLabel } from '@/lib/schema-ap-nde';
import { pickLiteral, selectValues, type Locale } from '@/lib/i18n';
...
/** Display name of the object in the interface language, or null when absent. */
function nameLiteral(obj: ObjectViewModel, locale: Locale): string | null {
  ...
  return pickLiteral(lits, locale)?.value ?? null;
}
...
function rowsFor(obj: ObjectViewModel): Row[] {
  ...
  return obj.fields
    ...
    .map((f) => ({ property: f.property, required: false, field: f }));
}

export function ObjectView({ obj }: { obj: ObjectViewModel }) {
  const locale = useLocale();
  const heading = nameLiteral(obj, locale);
  ...
  return (
    <article className="space-y-5">
      ...
      <dl className="divide-y divide-nde-line rounded-2xl border border-nde-line bg-white">
        {rows.map((r) => {
          const description = propertyDescription(obj.type, r.property, locale);
          return (
            <div key={r.property} className="grid grid-cols-1 gap-1 px-5 py-3 sm:grid-cols-[12rem_1fr] sm:gap-4">
              <dt className="flex items-start text-sm font-medium text-nde-muted sm:justify-end sm:text-right">
                {propertyLabel(obj.type, r.property, locale)}
                {description && <PropertyInfo text={description} />}
              </dt>
              <dd className="min-h-[1.25rem] space-y-2 text-sm text-nde-ink">
                {r.field
                  ? selectValues(r.field.values, locale, r.property).map((v, i) => (
                      <div key={i}>
                        <FieldValue value={v} />
                      </div>
                    ))
